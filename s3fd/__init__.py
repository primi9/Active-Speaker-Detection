    ...
    def detect_faces_gpu(self, images, s = 0.25, conf_th = 0.8):
        ...
        with torch.no_grad():
            for image in images:
                ...
                scaled_image = cv2.resize(image, dsize=(0, 0), fx=s, fy=s, interpolation=cv2.INTER_LINEAR)
            ...
            for item in range(n_frames):
                ...
                for i in range(detections.size(1)):
                    j = 0
                    while detections[item, i, j, 0] > conf_th:
                        score = detections[item, i, j, 0]
                        pt = (detections[item, i, j, 1:] * scale).cpu().numpy()
                        bbox = (pt[0], pt[1], pt[2], pt[3], score)
                        bboxes = np.vstack((bboxes, bbox))
                        j += 1

                keep = nms_(bboxes, 0.1)
                outputs.append(bboxes[keep])
